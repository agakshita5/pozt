...
EXPECTED_RUN_COLUMNS = {
    "id", "user_id", "title", "source_type", "source", "source_hash",
    ...
    with closing(connect()) as conn:
        ...
        if existing and existing != EXPECTED_RUN_COLUMNS:
            if os.getenv("ALLOW_DB_RESET") != "1":
                raise RuntimeError(
                    f"The runs table has columns {sorted(existing)} but the code expects "
                    f"{sorted(EXPECTED_RUN_COLUMNS)}. Back up {DB_PATH} and rerun with "
                    "ALLOW_DB_RESET=1 to drop and recreate, or migrate by hand."
                )
...
def source_hash(user_id: str, source_type: str, source: str) -> str:
    normalized = source.strip()
    if source_type == "url":
        normalized = normalized.rstrip("/").lower()
    else:
        normalized = " ".join(normalized.split())
    return hashlib.sha256(f"{user_id}:{source_type}:{normalized}".encode()).hexdigest()

# RUNS table

def insert_run(run_id: str, user_id: str, title: str, source_type: str, source: str, extracted_text: str, truncated: bool, tone: str) -> None:
    with closing(connect()) as conn, conn:
        conn.execute(
            "INSERT INTO runs (id, user_id, title, source_type, source, source_hash, extracted_text, truncated, tone, created_at) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (run_id, user_id, title, source_type, source, source_hash(user_id, source_type, source), extracted_text, int(truncated), tone, now()),
        )

def find_run_by_source(user_id: str, source_type: str, source: str) -> dict | None:
    with closing(connect()) as conn, conn:
        row = conn.execute(
            "SELECT * FROM runs WHERE source_hash = ?",
            (source_hash(user_id, source_type, source),),
        ).fetchone()
...
def list_runs(user_id: str) -> list[dict]:
    with closing(connect()) as conn, conn:
        rows = conn.execute(
            "SELECT id, title, source_type, tone, created_at FROM runs WHERE user_id = ? "
...
def delete_run(run_id: str, user_id: str) -> bool:
    with closing(connect()) as conn, conn:
        owned = conn.execute(
            "SELECT 1 FROM runs WHERE id = ? AND user_id = ?", (run_id, user_id)
        ).fetchone()
        if owned is None:
            return False
